...
export const getRecommendation = (s) => {
  if (s.is_flaky || s.is_visual)
    return { text: 'High Risk', class: 'bg-rose-100 text-rose-700 border-rose-200', accent: '#f43f5e' };
  const savings = calcNetSavings(s);
  if (savings <= 0)
    return { text: 'Keep Manual', class: 'bg-slate-100 text-slate-500 border-slate-200', accent: '#94a3b8' };
  const roi = s.dev_hours / savings;
  if (roi <= 3) return { text: 'Automate', class: 'bg-emerald-100 text-emerald-700 border-emerald-200', accent: '#10b981' };
  if (roi <= 6) return { text: 'Viable', class: 'bg-indigo-100 text-indigo-700 border-indigo-200', accent: '#6366f1' };
  return { text: 'Low Priority', class: 'bg-amber-100 text-amber-700 border-amber-200', accent: '#f59e0b' };
};
